...
\begin{alltt}
\hlstd{linear} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{data}\hlstd{,} \hlkwc{s} \hlstd{=} \hlkwd{frequency}\hlstd{(data),} \hlkwc{seasons_to_check} \hlstd{=} \hlnum{1}\hlopt{:}\hlstd{s) \{}
  \hlkwd{require}\hlstd{(forecast)}

  \hlcom{# Decompose the time series using STL}
  \hlstd{ts_data} \hlkwb{<-} \hlkwd{ts}\hlstd{(data,} \hlkwc{frequency} \hlstd{= s)}
  \hlstd{decomposed_data} \hlkwb{<-} \hlkwd{stl}\hlstd{(ts_data,} \hlkwc{s.window}\hlstd{=}\hlstr{"periodic"}\hlstd{)}

  \hlcom{# Extract the trend, seasonal, and random components}
  \hlstd{trend} \hlkwb{<-} \hlkwd{as.numeric}\hlstd{(decomposed_data}\hlopt{$}\hlstd{time.series[,} \hlstr{"trend"}\hlstd{])}
  \hlstd{seasonal} \hlkwb{<-} \hlkwd{as.numeric}\hlstd{(decomposed_data}\hlopt{$}\hlstd{time.series[,} \hlstr{"seasonal"}\hlstd{])}
  \hlstd{remainder} \hlkwb{<-} \hlkwd{as.numeric}\hlstd{(decomposed_data}\hlopt{$}\hlstd{time.series[,} \hlstr{"remainder"}\hlstd{])}

  \hlcom{# De-trended series (seasonal + noise)}
  \hlstd{detrended} \hlkwb{<-} \hlstd{seasonal} \hlopt{+} \hlstd{remainder}

  \hlcom{# Initialize Ui, Vi, and di}
  \hlstd{Ui} \hlkwb{<-} \hlkwd{numeric}\hlstd{(}\hlkwd{length}\hlstd{(seasons_to_check))}
  \hlstd{Vi} \hlkwb{<-} \hlkwd{numeric}\hlstd{(}\hlkwd{length}\hlstd{(seasons_to_check))}
  \hlstd{di} \hlkwb{<-} \hlkwd{numeric}\hlstd{(}\hlkwd{length}\hlstd{(seasons_to_check))}

  \hlcom{# Calculate Ui and Vi for each specified seasonal period}
  \hlkwa{for} \hlstd{(index} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(seasons_to_check)) \{}
    \hlstd{i} \hlkwb{<-} \hlstd{seasons_to_check[index]}
    \hlstd{S_j} \hlkwb{<-} \hlstd{detrended[}\hlkwd{seq}\hlstd{(i,} \hlkwd{length}\hlstd{(data),} \hlkwc{by} \hlstd{= s)]}
    \hlstd{j} \hlkwb{<-} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(S_j)}

    \hlcom{# Only proceed if S_j has a sufficient size (at least two non-NA values)}
    \hlkwa{if} \hlstd{(}\hlkwd{sum}\hlstd{(}\hlopt{!}\hlkwd{is.na}\hlstd{(S_j))} \hlopt{>} \hlnum{1}\hlstd{) \{}
